<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rounded&height=220&color=gradient&text=Kamboji%20Venkata%0ASubramanyam&fontSize=50&animation=twinkling" />
</p>


<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=800&color=FFC300&center=true&vCenter=true&width=600&lines=CSE+(Data+Science)+Graduate;Full-Stack+%26+ML+Developer;Building+Real-World+AI+Projects;Open+to+Collaborations+%26+Internships;Always+Learning+%F0%9F%9A%80" alt="Typing SVG" />
</p>

<p align="center">
🚀 I’m learning to build scalable full-stack applications while exploring Machine Learning to solve real-world problems.
</p>

---

## 👨‍💻 About Me

- 🎓 Computer Science and Engineering (Data Science) Graduate from Rajeev Gandhi Memorial College of Engineering and Technology (RGM CET), Nandyal, India.
- 🎓 Currently working as  Student Research intern at CDAC- Hyderabad 
- 💡 Passionate about **Full-Stack Web Development** & **Machine Learning**, focused on real-world AI solutions  
- 🌱 Currently exploring **React.js**, **Node.js**, and strengthening **ML & System Design** fundamentals  
- 🤝 Actively seeking **internship opportunities** and **open-source collaborations**  
- 📌Portfolio: [Subramanyam's Portfolio](https://venkatasubramanyamkamboji.vercel.app/)
- 📫 Email: **subramanyamkamboji@gmail.com**

---

## ⭐ Highlights

- 🏆 1st Place – Avishkar 2K24, Pulla Reddy Engineering College | 300+ participants
- 📘 Completed – Introduction to Generative AI Learning Path, Google Skill Boost
- 🚀 Built and deployed **frontend applications & deployed by using github pages**
- 🧠 Strong hands-on experience in **Machine Learning, NLP & LLM integrations**
-👥 Lead – HYDRA Club, led bootcamp-style sessions, mentored 60+ juniors, and organized coding challenges & workshops.
---

## 🚀 Skills

### 🧑‍💻 Programming Languages
[![C](https://img.shields.io/badge/C-000000?style=for-the-badge&logo=c&logoColor=00599C)](https://en.cppreference.com/w/c)
[![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=3776AB)](https://docs.python.org/3/)
[![Java](https://img.shields.io/badge/Java-000000?style=for-the-badge&logo=openjdk&logoColor=ED8B00)](https://docs.oracle.com/javase/)
[![JavaScript](https://img.shields.io/badge/JavaScript-000000?style=for-the-badge&logo=javascript&logoColor=F7DF1E)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)

---

### 🎨 Frontend Development
[![HTML5](https://img.shields.io/badge/HTML5-000000?style=for-the-badge&logo=html5&logoColor=E34F26)](https://developer.mozilla.org/en-US/docs/Web/HTML)
[![CSS3](https://img.shields.io/badge/CSS3-000000?style=for-the-badge&logo=css3&logoColor=1572B6)](https://developer.mozilla.org/en-US/docs/Web/CSS)
[![Basic React](https://img.shields.io/badge/React-000000?style=for-the-badge&logo=react&logoColor=61DAFB)](https://react.dev/)



### 🗄️ Databases & ORM
[![MongoDB](https://img.shields.io/badge/MongoDB-000000?style=for-the-badge&logo=mongodb&logoColor=47A248)](https://www.mongodb.com/)
[![MySQL](https://img.shields.io/badge/MySQL-000000?style=for-the-badge&logo=mysql&logoColor=4479A1)](https://dev.mysql.com/doc/)

---

### ☁️ Cloud & Deployment
[![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://vercel.com/)
[![Google Cloud](https://img.shields.io/badge/Google%20Cloud-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white)](https://cloud.google.com/)
[![Netlify](https://img.shields.io/badge/Netlify-000000?style=for-the-badge&logo=netlify&logoColor=00C7B7)](https://www.netlify.com/)


---

### 🔧 Tools & Platforms
[![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git&logoColor=F05032)](https://git-scm.com/)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/)
[![VS Code](https://img.shields.io/badge/VS_Code-000000?style=for-the-badge&logo=visual-studio-code&logoColor=007ACC)](https://code.visualstudio.com/)
[![Google Cloud](https://img.shields.io/badge/Google%20Cloud-000000?style=for-the-badge&logo=googlecloud&logoColor=4285F4)](https://cloud.google.com/)




---

## 🚀 Featured Projects

📊 ChurnXpert – Customer Churn Prediction System

A machine learning–based system to predict customer churn and support data-driven retention strategies.

🧹 Performed data cleaning & preprocessing using Pandas and NumPy

⚙️ Applied feature engineering to improve model performance

🤖 Built and trained ML models using Scikit-learn

📈 Achieved 89% prediction accuracy

📊 Visualized insights using Matplotlib & Seaborn

🧪 Evaluated models using standard performance metrics

🛠 Tech Stack:
Python · Pandas · NumPy · Scikit-learn · Matplotlib · Seaborn · Jupyter Notebook

📈 Outcome:
Enabled accurate churn prediction to help businesses proactively retain customers through data-driven insights.
---

🤖 WikiBot – Web-Based Intelligent Chatbot

A lightweight chatbot that fetches real-time information from Wikipedia based on user queries.

🔎 Retrieves real-time data from Wikipedia based on user input

💬 Interactive and responsive UI built with HTML, CSS, and JavaScript

⚡ Processes user queries using keyword-based logic

🌐 Successfully deployed on GitHub Pages

🛠 Tech Stack:
HTML · CSS · JavaScript · GitHub Pages

📈 Outcome:
Delivered a simple, accessible chatbot that provides quick and accurate information through a web-based interface.

---


## 📈 GitHub Stats

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api?username=VenkataSubramanyamKamboji&show_icons=true&theme=tokyonight&hide_border=true&count_private=true"
    width="48%"
  />
  <img
    src="https://streak-stats.demolab.com?user=VenkataSubramanyamKamboji&theme=tokyonight&hide_border=true"
    width="48%"
  />
</p>

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=VenkataSubramanyamKamboji&layout=compact&theme=tokyonight&hide_border=true"
    width="40%"
  />
</p>



---

## 🎯 Open To

- Software Engineer Internships  
- Full-Stack Developer Roles  
- ML / Data Science Internships  
- Open-source collaborations  

---

## 📫 Let's Connect


[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/VenkataSubramanyamKamboji)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-000000?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://www.linkedin.com/in/venkatasubramanyamkamboji/)
[![Email](https://img.shields.io/badge/Email-000000?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:subramanyamkamboji@gmail.com)


---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:6A11CB,100:2575FC&height=150&section=footer&text=Thanks%20for%20Visiting!%20🚀&fontSize=30&fontColor=ffffff&animation=fadeIn" />
</p>
